...
namespace IPFees.Web.Pages
{
    ...
    {        
        ...
        public async Task<IActionResult> OnGetAsync()
        {
            ...
            Vars = _calc.GetInputs().Select(pv => new ParsedVariableViewModel(pv.Name, pv.GetType().ToString(), pv, string.Empty, Array.Empty<string>(), 0, false, DateOnly.MinValue)).ToList();
            return Page();
        }
    }

    public record ParsedVariableViewModel(string Name, string Type, DslInput Var, string StrValue, string[] ListValue, double DoubleValue, bool BoolValue, DateOnly DateValue);
}
